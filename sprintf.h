	...
	namespace strings
	{
		...
		*/
		struct printf_format
		{
			...
			template<typename T>
			typename std::enable_if<std::is_floating_point<T>::value || maxy::traits::is_explicitly_constructible<long double, T>::value, size_t>::type
			...
			template<typename T>
			typename std::enable_if<std::is_integral<T>::value || maxy::traits::is_explicitly_constructible<long long, T>::value, size_t>::type
				output (std::ostream & out, T arg)
			{
				std::ostringstream os;
				prepare_stream (os);
			...
			template<typename T>
			typename std::enable_if<!std::is_arithmetic<T>::value && !maxy::traits::is_explicitly_constructible<long double, T>::value, size_t>::type
				output (std::ostream & out, T arg)
			{
				std::ostringstream os;
				prepare_stream (os);
		...
		size_t osprintf (std::ostream & out, const char * format, T arg, Ts... args)
		{
			...
			while (*format)
			{
				...
				else
				{
					...
					count += f.output (out, arg);
					count += maxy::strings::osprintf (out, format, args...);
					return count;
				}
			}
			return count;
